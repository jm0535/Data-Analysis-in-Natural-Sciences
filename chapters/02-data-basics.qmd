...
Most data analysis is data wrangling with a bit of statistics on the end. This chapter is about the wrangling part: getting numbers and text into R, working out what type they are, cleaning the messy bits, and reshaping things into a form the rest of the book can work with. None of it is glamorous, all of it is essential.

## Understanding data structures

Before any analysis, you need to know what kind of thing R thinks it is holding. A column of numbers behaves very differently from a column of factors, and a date that R reads as a string is a date you cannot plot on a timeline.

### Data types

R has a handful of basic types. You will meet all of them in the first hour of any project.
...
#| label: fig-eda-plots
#| fig-cap:
#|   - "Distribution of penguin bill lengths"
#|   - "Body mass distribution by penguin species"
#|   - "Relationship between bill length and flipper length"
#| fig-alt:
#|   - "Histogram of penguin bill length (mm) with 1mm bins; the distribution is roughly bimodal with peaks near 39mm and 47mm and a range of about 32-60mm."
#|   - "Boxplot of body mass (g) by penguin species; Gentoo penguins are clearly heaviest (median ~5000g) while Adelie and Chinstrap have similar lower medians (~3700g)."
#|   - "Scatter plot of flipper length (mm) vs bill length (mm) colored by species; Gentoo points cluster at longer flippers (>210mm), while Adelie and Chinstrap separate mainly by bill length."
