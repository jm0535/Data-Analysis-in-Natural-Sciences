...
If you work with data in any of the natural sciences, sooner or later you have to stop eyeballing spreadsheets and start analysing them properly [@wickham2016r; @zuur2009mixed]. This chapter is where we begin. We will cover what data analysis is really for in this field, how to set up R and RStudio on your machine, and what the tidyverse and tidymodels packages give you that base R does not.
...
Most of the choices field scientists, agencies, and resource managers make are decisions under uncertainty. Which patch of forest do we protect first? Is this season's wheat yield really lower, or are we looking at noise? Good analysis turns a heap of measurements into something you can act on. The kinds of questions it answers in practice:

- Which areas and species to prioritise for conservation
- How to manage a resource so it lasts
- What is happening to crop yields, and why
- Whether a pollution control measure is actually working
- How an ecosystem might respond as the climate shifts

### Patterns hide in the noise

Looking at a column of numbers tells you very little. Plotting it and running a few summaries often reveals structure you would never spot by eye. This is true in ecology, geology, marine science, climatology, agriculture, and every other field where data come in messy and the signal is not loud.

### Hypotheses need testing

A hypothesis is just a guess until you put numbers on it. Statistical tests give you a defensible answer to the question "could this pattern have come from chance?" That answer is rarely the whole story, but without it you are arguing from impression.

### Predicting what comes next

Modelling lets you go beyond describing the present and start estimating what is likely to happen. Where will a species move as its range warms? What is next season's likely yield given the rainfall so far? Which lakes are at risk of an algal bloom? These are not crystal-ball questions, they are regression and forecasting problems.
...
The tidyverse is a family of R packages that share a common grammar and a common idea of what tidy data looks like. Once you get used to it, code reads almost like instructions to a colleague.

Four ideas it leans on:

1. Tidy data: one variable per column, one observation per row.
2. Pipes: chain steps together with `%>%` or `|>` so the data flows top to bottom.
3. Consistent functions that play well with each other.
4. Code you can read without a decoder ring.
...
#| fig-alt: "Boxplot with jittered points showing bill length (mm) on the y-axis for three penguin species; Adelie has the shortest bills (~35-45mm) while Chinstrap and Gentoo have longer overlapping ranges (~40-55mm)."
#| fig-width: 8
#| fig-height: 5
#| warning: false
...
#| fig-alt: "Boxplot with jittered points showing body mass (g) by penguin species; Gentoo penguins are notably heavier (~4500-6000g) than Adelie and Chinstrap (~3000-4500g), which have overlapping distributions."
...
#| fig-alt: "Scatter plot of bill length (mm) vs body mass (g) colored by species, with linear regression lines and confidence bands; all three species show positive slopes, with Gentoo points clustered in the upper right."
