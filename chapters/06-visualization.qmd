...
1.  Understand the importance of data visualization in scientific communication
2.  Choose appropriate plot types for different data structures (categorical, numerical, spatial)
3.  Create publication-quality bar charts, histograms, and scatter plots using `ggplot2`
4.  Visualize distributions and group comparisons with box plots
5.  Explore complex relationships using heatmaps and time series plots
6.  Apply best practices for design, accessibility, and data integrity in visualizations
:::

A good plot does in a second what three paragraphs of text struggle to do. A bad plot does the opposite. This chapter is about making more of the first kind. We will work through the chart types you will use most often in natural sciences (bars, histograms, boxplots, scatter, time series, heatmaps), how to pick the right one for what you are showing, and how to push a default ggplot output into something you would not be embarrassed to put in a paper or report.

::: {.callout-tip}
## Accessibility note

Every figure in this book pairs a `fig-cap` (visible caption) with a `fig-alt`
(alternative text read by screen readers). When you build your own figures,
add `fig-alt` describing the *content and main message* of the plot —
not just the chart type. We also default to the **viridis** colour palette
because it is perceptually uniform and remains distinguishable for readers
with deuteranopia and protanopia (the two most common forms of colour-vision
deficiency).
:::
...
1. Patterns jump out. A trend, an outlier or a population crash that is invisible in a table is obvious in a scatter or time-series plot. Your eye is doing real work the moment you look at a good chart.

2. Other people can follow your reasoning. Most of your audience will never read your code, and many will not read your prose either. The figures are what they remember.

3. You spot problems early. Half the value of plotting is catching the data error you would otherwise have run a test on.
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Bar chart showing the conservation status of plant species across different regions"
#| fig-alt: "Grouped bar chart with conservation status categories (Endangered, Vulnerable, Least Concern) on the x-axis and species counts on the y-axis, with bars colored by region."
...
The bar chart in @fig-bar-chart reveals important patterns:
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Scatter plot showing relationship between threat scores and year last seen"
#| fig-alt: "Scatter plot with year last seen on the x-axis and threat score on the y-axis. Points are colored by taxonomic group; a fitted trend line indicates the overall relationship."
# Load necessary data
library(tidyverse)
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Box plot comparing threat scores across different continents"
#| fig-alt: "Side-by-side box plots, one per continent, showing the distribution of threat scores. Each box displays the median, interquartile range, and outliers as points beyond the whiskers."
...
#| fig-width: 9
#| fig-height: 7
#| fig-cap: "Heatmap visualizing the correlation matrix between different threat types"
#| fig-alt: "Square correlation heatmap with threat types on both axes. Cell colors range from blue (negative correlation) through white (zero) to red (positive correlation); numeric correlation coefficients are overlaid on each cell."
# Create a simulated biodiversity dataset
...
#| fig-width: 9
#| fig-height: 6
#| fig-cap: "Time series plot tracking agricultural trends over time for major producers"
#| fig-alt: "Line plot with year on the x-axis and crop yield on the y-axis. Separate colored lines track each major producing country over multiple decades, showing long-term trends and divergence between countries."
