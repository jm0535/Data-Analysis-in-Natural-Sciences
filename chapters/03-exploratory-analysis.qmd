...
Before you run a single test, you look at your data. That is exploratory data analysis, and it is the part most people skip and then regret. This chapter walks through what to look for when you first open a dataset, how to spot trouble (missing values, weird outliers, columns that should not be character), and how to use simple plots and summaries to decide what questions are even worth asking.

## What EDA is actually for

Five honest reasons to spend an afternoon on EDA before you spend a week on modelling:

1. You find out what the dataset is. Variable types, ranges, how the file is structured.
2. You spot quality problems early. Missing values, duplicated rows, impossible measurements.
3. You see patterns. Distributions, relationships, trends across time or space.
4. You generate hypotheses worth testing, instead of inventing them to match a result.
5. You pick the right analysis. The right plot first usually tells you whether the right test is parametric or not.
...
```{r}
#| label: tbl-wheat-summary-stats
#| tbl-cap: "Summary Statistics for Global Wheat Yields"
...
#| fig-cap: "Distribution of global wheat yields"
#| fig-alt: "Histogram of global wheat yields (tonnes per hectare) with 30 forest-green bins; the distribution is right-skewed with most observations between 1 and 4 tonnes/ha and a long tail extending toward higher yields."
...
```{r}
#| label: tbl-top-wheat-countries
#| tbl-cap: "Top 10 Countries by Average Wheat Yield"
...
- The histogram in @fig-descriptive-stats confirms this skewed distribution, with most countries clustered at lower to moderate yield levels, and fewer countries achieving very high yields.
- The top 10 countries listed in @tbl-top-wheat-countries show which nations have the most productive wheat cultivation systems, likely due to advanced agricultural practices, favorable climate conditions, or intensive farming methods.
...
```{r}
#| label: fig-boxplots-summary
#| fig-cap:
#|   - "Wheat yields by country"
#|   - "Enhanced boxplot with data points"
#| fig-alt:
#|   - "Boxplot of wheat yield (tonnes/ha) for six major producing countries (US, China, India, Russia, France, Australia); France and the US show the highest medians while Australia and Russia have the lowest and most variable yields."
#|   - "Boxplot with overlaid jittered points showing wheat yield by country; the individual points reveal sample density within each country's distribution, with clear separation between higher-yielding (France, China) and lower-yielding (Australia, Russia) producers."
...
The box plots in @fig-boxplots-summary reveal important comparisons between major wheat-producing countries:
...
```{r}
#| label: fig-barchart-yields
#| fig-cap: "Top 10 countries by average wheat yield (2010-present)"
#| fig-alt: "Vertical bar chart of the top 10 countries by average wheat yield since 2010, sorted ascending; bars are dark green and country names are rotated 45 degrees on the x-axis, with the leading country reaching roughly 8-9 tonnes/ha."
...
The bar chart in @fig-barchart-yields reveals the top 10 countries by average wheat yield:
...
#|   - "Combined histogram with density overlay"
#| fig-alt:
#|   - "Histogram of global wheat yields (tonnes/ha) in 30 dark-green bins; right-skewed distribution with most values between 1 and 4 tonnes/ha and a long thin tail extending past 8 tonnes/ha."
#|   - "Smoothed density curve of wheat yields shaded dark green; a single broad peak around 2-3 tonnes/ha with a long right tail confirms right-skewed global production."
#|   - "Histogram on density scale with a smooth density curve overlaid; the curve traces the right-skewed shape of the bars, peaking near 2-3 tonnes/ha and tapering past 7 tonnes/ha."
...
The distribution visualizations in @fig-distributions reveal key patterns in wheat yields:
...
```{r}
#| label: fig-boxplots-visualizing
#| fig-cap:
#|   - "Country comparison boxplot"
#|   - "Boxplot with jittered observations"
#| fig-alt:
#|   - "Boxplot of wheat yield (tonnes/ha) since 2000 for six major producers; France and the US show the highest medians, Russia and Australia the lowest, with rotated country names on the x-axis."
#|   - "Boxplot of country-level wheat yield with jittered individual-year points overlaid in dark green, exposing both summary statistics and per-year spread within each country."
...
The box plots in @fig-boxplots-visualizing reveal important comparisons between major wheat-producing countries:
...
```{r}
#| label: fig-barchart-distributions
#| fig-cap: "Top 10 countries by average wheat yield"
#| fig-alt: "Vertical dark-green bar chart of the ten highest-yielding wheat countries since 2010, reordered ascending so the tallest bar (top producer) appears on the right; country names are rotated 45 degrees."
...
The bar chart in @fig-barchart-distributions reveals the top 10 countries by average wheat yield:
...
#|   - "Wheat vs rice yields by continent"
#| fig-alt:
#|   - "Scatter plot of country-year wheat yield (x) vs rice yield (y) since 2000 in dark green semi-transparent points; the cloud shows a weak positive association with high density at low-to-moderate yields for both crops."
#|   - "Scatter plot of wheat vs rice yields colored by continent (Africa, Asia, Europe, North America, Oceania, South America); continents cluster differently, with European points high on wheat and Asian points high on rice."
...
The scatter plots in @fig-scatter-relationships reveal the relationship between wheat and rice yields:
...
#| fig-cap: "Correlation matrix of crop yields"
#| fig-alt: "Upper-triangle correlation plot rendered with corrplot using filled circles whose size and color encode the Pearson correlation between six crops (wheat, rice, maize, soybeans, potatoes, beans); most pairs are positively correlated."
...
The correlation matrix in @fig-correlation-matrix reveals the strength and direction of relationships between variables:
...
```{r}
#| label: fig-pair-plots
#| fig-cap:
#|   - "Basic pair plot of crop yields"
#|   - "Enhanced pair plot with GGally"
#| fig-alt:
#|   - "Base R pair plot matrix of six crop-yield variables in dark-green points; each off-diagonal cell is a scatter plot of one crop against another, revealing positive associations across crop pairs."
#|   - "GGally ggpairs matrix showing density plots on the diagonal, scatter plots below the diagonal, and Pearson correlations above the diagonal for the six crop-yield variables."
...
The pair plots in @fig-pair-plots reveal the relationships between multiple variables:
...
#| fig-cap: "Box plot showing potential outliers in wheat yields"
#| fig-alt: "Single vertical boxplot of global wheat yields (tonnes/ha) in dark green; the box covers roughly 1.5-4 tonnes/ha with many outlier dots extending well above the upper whisker, indicating right-skewed extremes."
...
The box plot and outlier detection in @fig-outlier-detection reveal potential outliers:
...
```{r}
#| label: fig-time-series-eda
#| fig-cap: "Wheat yield trends over time for major producers"
#| fig-alt: "Line and point time series of annual wheat yield since 1960 for the United States, China, India, and France, colored with the Dark2 palette; all four series trend upward, with France climbing the fastest to the highest yields."
...
The time series plot in @fig-time-series-eda reveals temporal patterns:
...
#| fig-alt:
#|   - "Horizontal naniar gg_miss_var bar chart ranking each crop_yields variable by the number of missing observations, with the most-missing variables at the top."
#|   - "Naniar vis_miss heatmap of Entity, Year, and the three main crop columns; black tiles mark missing cells, revealing that maize and rice yields have substantial blocks of missingness while Entity and Year are complete."
...
The missing data analysis in @fig-missing-data reveals patterns of missingness:
...
1. Load the plant biodiversity dataset from `../data/ecology/biodiversity.csv` and perform a comprehensive exploratory analysis.
...
6. Perform the same exploratory analyses in R for the spatial dataset in `../data/geography/spatial.csv`.

```{r}
#| label: fig-conservation-status
#| fig-cap: "Conservation status of plant species by region (illustrative simulated data)"
#| fig-alt: "Stacked bar chart with six geographic regions on the x-axis and number of species on the y-axis. Each bar is segmented by IUCN conservation status (Least Concern, Near Threatened, Vulnerable, Endangered, Critically Endangered) using the viridis palette."
...
# Note: this example uses *simulated* data so the chapter remains
# reproducible without the full ecology dataset. For the exercises above,
# load the real file with:
#   plant_data <- readr::read_csv("../data/ecology/biodiversity.csv")
