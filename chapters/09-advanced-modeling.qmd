...
This chapter is about two things that did not fit neatly into earlier ones. The first is model interpretation: when you fit a random forest or a gradient boost and it predicts well, how do you actually explain what it is doing? The second is time series, which behaves badly in ordinary regression because the observations are not independent. Both come up often enough in real ecological work that ignoring them is no longer a defensible choice.

## Model interpretation and explainability

A random forest can be embarrassingly accurate and equally opaque. You can predict species presence with 90% accuracy and still have no idea which variables are driving the prediction. That is fine for a competition but useless for a paper or a management recommendation. The methods in this section pry open the black box: which variables matter, how the response changes with each predictor, and why a particular observation was classified the way it was.
...
#| message: false
#| warning: false
#| fig-cap: "Variable importance for body mass prediction"
#| fig-alt: "DALEX permutation variable-importance boxplot for the random forest predicting body mass; bars/segments rank predictors by drop in RMSE when permuted, with flipper length and species emerging as the most important."
...
#| fig-cap:
#|   - "Partial dependence: Flipper length"
#|   - "Partial dependence: Bill length"
#| fig-alt:
#|   - "DALEX partial-dependence line plot of predicted body mass (g) as flipper length (mm) varies, averaging over other predictors; shows a strongly increasing relationship."
#|   - "DALEX partial-dependence line plot of predicted body mass (g) across the range of bill length (mm), showing a more modest positive average effect than flipper length."
...
#|   - "ICE plots with partial dependence overlay"
#| fig-alt:
#|   - "ICE plot from DALEX: 50 individual curves showing how predicted body mass changes with flipper length for each penguin, revealing heterogeneity across individuals."
#|   - "Same ICE curves with the average partial-dependence profile overlaid in yellow, contrasting individual trajectories against the global mean effect."
...
```{r}
#| label: fig-individual-predictions
#| fig-cap:
#|   - "Break-down plot for individual prediction"
#|   - "SHAP values for individual prediction"
#| fig-alt:
#|   - "DALEX break-down bar plot for a single test penguin, showing each predictor's positive or negative contribution to the final predicted body mass, starting from the mean intercept."
#|   - "SHAP value plot for the same individual prediction averaged across 25 random variable orderings, with bars indicating each feature's average contribution to the body-mass prediction."
...
#| message: false
#| fig-cap:
#|   - "Simulated population time series"
#|   - "Autocorrelation and partial autocorrelation functions"
#| fig-alt:
#|   - "Steel-blue line-and-point time series of simulated annual population size from 1990 to 2023, showing a clear upward trend with a five-year cyclic oscillation."
#|   - "Stacked base-R ACF and PACF plots of the simulated population series; the ACF decays slowly indicating strong autocorrelation, and the PACF shows a large spike at lag 1."
...
#| fig-cap:
#|   - "Time series decomposition"
#|   - "Decomposition components visualization"
#| fig-alt:
#|   - "Base-R STL decomposition output stacking observed, seasonal (5-year cycle), trend, and remainder panels for the simulated population series."
#|   - "ggplot facet_wrap version of the STL decomposition with observed, trend, seasonal, and remainder series drawn as steel-blue lines in separate vertically stacked panels with free y-axes."
...
#| fig-cap: "Population forecast with ARIMA model"
#| fig-alt: "forecast::autoplot of the ARIMA model: historical population line continues into a five-year fan forecast with shaded 80% and 95% prediction intervals widening into the future."
...
#| warning: false
#| message: false
#| fig-cap: "Phenological shift: First flowering date over time"
#| fig-alt: "Scatter plot of simulated first-flowering day-of-year from 1980-2023 in dark-green points with a red linear regression line and confidence band sloping downward, illustrating earlier flowering attributable to climate warming."
