...
Conservation work is where the statistics in this book stop being academic. Decisions about protected areas, threatened species recovery, and management interventions cost money and sometimes determine whether a population persists. This chapter brings the tools from earlier chapters (descriptive statistics, regression, mixed-effects models, the machine learning from Chapter 9) to bear on the questions practitioners actually face: is the population in trouble, is the intervention working, and where should the next dollar go?
...
#| echo: false
#| tbl-cap: "Common Data Sources in Conservation Science"
library(knitr)
...
```{r}
#| label: fig-sdm-analysis
#| eval: true
#| warning: false
#| message: false
#| fig-cap:
#|   - "Species presence/absence in environmental space"
#|   - "Predicted probability surface from SDM"
#|   - "SDM predictions with observed presence points"
#| fig-alt:
#|   - "Scatter of simulated sites in temperature (x) vs precipitation (y) space, with red points marking species absences and blue points marking presences; presences concentrate at moderate temperatures and high precipitation."
#|   - "Tile heatmap of logistic-SDM predicted occurrence probability across the temperature-precipitation grid (elevation held at its mean), shaded with the viridis plasma palette from low (dark) to high (yellow)."
#|   - "Same plasma probability heatmap with white-outlined circles overlaid for observed presence locations, which cluster in the high-probability (bright) region of environmental space."
...
The species distribution modeling approach in @fig-sdm-analysis reveals several important insights for conservation:
...
```{r}
#| label: fig-spatial-autocorrelation
#| eval: true
#| warning: false
#| message: false
#| fig-cap: "Spatially autocorrelated species abundance across sampling sites"
#| fig-alt: "Equal-aspect map of 50 sampling site locations on an x-y grid, with point color and size encoding simulated species abundance using a viridis scale; nearby sites tend to share similar values, visualizing positive spatial autocorrelation."
...
```{r}
#| label: fig-moran-correlogram
#| eval: true
#| warning: false
#| message: false
#| fig-cap: "Spatial correlogram showing Moran's I at different distance classes"
#| fig-alt: "Line-and-point spatial correlogram of Moran's I (y) against distance class (x) in steel blue, with a dashed zero reference line and points colored red where p<0.05; autocorrelation is highest at short distances and decays toward zero."
...
The spatial autocorrelation analysis in @fig-spatial-autocorrelation and @fig-moran-correlogram reveals critical information for ecological modeling:
...
#| warning: false
#| message: false
#| fig-cap: "Comparison of residual patterns from naive vs spatial lag models"
#| fig-alt: "Patchwork stack of two maps: top panel shows naive lm residuals with visible spatial clustering of red (positive) and blue (negative) points; bottom panel shows spatial lag model residuals with much less spatial pattern."
...
#| fig-cap:
#|   - "Population trajectories across monitoring sites"
#|   - "Observed vs predicted population sizes from mixed model"
#| fig-alt:
#|   - "Line-and-point time series of simulated annual population counts for 10 monitoring sites colored differently; all sites show a general downward trend over 15 years with site-level variation in baseline level."
#|   - "Scatter of observed annual population counts colored by site, with black per-site fitted lines from a linear mixed-effects model overlaid, showing the model-estimated declining trajectory at each site."
...
The population trend analysis in @fig-population-trends provides several important insights for conservation management:
...
```{r}
#| label: fig-landscape-fragmentation
#| eval: true
#| warning: false
#| message: false
#| fig-cap:
#|   - "Simulated landscape with four land cover classes"
#|   - "Percentage of landscape by land cover class"
#|   - "Number of patches by land cover class"
#|   - "Edge density by land cover class"
#| fig-alt:
#|   - "Base-R terra plot of a 30x30 raster colored by four land-cover classes (forest green, agriculture yellow, water blue, urban grey) showing a mosaic pattern of cells."
#|   - "Bar chart of percentage of landscape (y) by land-cover class (x); bars are colored to match the landscape map, with forest tallest at 40% and urban shortest at 10%."
#|   - "Bar chart of number of discrete patches per land-cover class (y); forest has the most patches (15) and urban the fewest (5), with colors matching the landscape map."
#|   - "Bar chart of edge density (y) by land-cover class (x) using the same color scheme; forest has the highest edge density, indicating it is the most fragmented class."
...
The landscape fragmentation analysis in @fig-landscape-fragmentation reveals several key insights for conservation planning:
...
```{r}
#| label: fig-baci-analysis
#| fig-cap:
#|   - "Biodiversity trends in protected vs unprotected sites"
#|   - "BACI interaction plot showing protection effect"
#| fig-alt:
#|   - "Time series of simulated biodiversity index by year with thin per-site lines colored by protection status and thick mean-trend lines for protected vs unprotected groups; a vertical dashed line at 2018 marks when protection began."
#|   - "BACI interaction plot of mean biodiversity index across Before/After periods with separate colored lines for protected and unprotected sites; the lines diverge after protection, indicating a positive intervention effect."
...
The BACI analysis in @fig-baci-analysis reveals crucial information about protected area effectiveness:
...
```{r}
#| label: fig-threat-assessment
#| fig-cap:
#|   - "Threat prioritization based on composite magnitude score"
#|   - "Component breakdown of threat assessment criteria"
#| fig-alt:
#|   - "Horizontal-style bar chart (rotated x-axis labels) of composite threat magnitude (severity × scope × irreversibility) in steel blue, ranked from Habitat Loss highest to Overexploitation lowest."
#|   - "Dodged bar chart showing severity, scope, and irreversibility scores side by side for each threat; Habitat Loss and Climate Change score uniformly high across criteria."
...
The threat prioritization analysis in @fig-threat-assessment reveals important insights for conservation planning:
...
```{r}
#| label: fig-conservation-planning
#| fig-cap:
#|   - "Species presence matrix across candidate sites"
#|   - "Priority sites identified through complementarity analysis"
#| fig-alt:
#|   - "pheatmap of a 10-site x 15-species presence/absence matrix with steel-blue cells for presence and white for absence; cell values are printed inside, showing the sparse distribution of species across sites."
#|   - "Same species-by-site pheatmap with a row annotation bar marking the three sites selected by the greedy complementarity algorithm as conservation priorities."
...
The complementarity analysis in @fig-conservation-planning provides key insights for conservation prioritization:
...
```{r}
#| label: fig-climate-vulnerability
#| fig-cap:
#|   - "Climate change vulnerability ranking by species"
#|   - "Component breakdown of vulnerability assessment"
#| fig-alt:
#|   - "Steel-blue bar chart of composite climate vulnerability scores for 12 species ranked from most to least vulnerable, with species names rotated 45 degrees on the x-axis."
#|   - "Dodged bar chart breaking each species' vulnerability into sensitivity, adaptive capacity, and exposure components in different fills, revealing which drivers dominate for each species."
...
The vulnerability assessment in @fig-climate-vulnerability reveals important patterns for climate adaptation planning:
...
```{r}
#| label: fig-community-monitoring
#| fig-cap: "Poaching incidents reported by community monitors over time"
#| fig-alt: "Two-panel faceted line-and-point time series (Year 1, Year 2) of monthly poaching incidents for five villages distinguished by color; series show seasonal oscillation and a slight downward trend over time."
...
The community monitoring analysis in @fig-community-monitoring reveals important patterns for conservation management:
...
#|   - "Distribution of plastic types in pollution"
#|   - "Top corporate contributors to plastic pollution"
#| fig-alt:
#|   - "Horizontal steel-blue bar chart of the top 15 countries ranked by total plastic items collected during cleanup events, with comma-formatted counts on the x-axis."
#|   - "Vertical bar chart of total plastic items by polymer type (HDPE, LDPE, PET, PP, PS, PVC, Other) with fill colors per type; PET dominates the distribution."
#|   - "Horizontal coral-colored bar chart of the top 10 branded parent companies by total plastic items collected, excluding unbranded and grand-total rows."
...
```{r}
#| label: fig-storm-analysis
#| warning: false
#| message: false
#| fig-cap:
#|   - "Annual storm frequency trend (1975-2020)"
#|   - "Storm observations by hurricane category"
#|   - "Spatial distribution of storm activity"
#| fig-alt:
#|   - "Dark-blue line of annual named-storm counts from 1975 to 2020 with a red LOESS smoother and confidence band suggesting a gradual increase in storm frequency over the period."
#|   - "Bar chart of storm observations grouped by Saffir-Simpson hurricane category (1-5) using the viridis plasma palette; lower categories have many more observations than the rarer high-intensity categories."
#|   - "Lat/long scatter of storm observation points with color encoding wind speed (inferno palette) and fixed aspect ratio, mapping the geographic footprint of storm activity."
