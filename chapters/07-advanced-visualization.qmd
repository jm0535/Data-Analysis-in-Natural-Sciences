...
Chapter 6 covered the workhorse charts. This one goes further: how to push a ggplot output to publication standard, how to stitch multiple panels into one figure, how to put data on a map, and how to make a plot a reader can actually click around in. Pick what you need; you do not have to learn all of it at once.

## Publication-quality graphics

### Customising ggplot2 themes

ggplot2 lets you change almost every visual element of a plot. The defaults are fine for exploration, but for a paper or report you usually want something cleaner.

```{r}
#| label: fig-custom-theme
#| fig-cap: "Customized ggplot2 scatter plot with professional theme"
#| fig-alt: "Scatter plot of iris sepal length vs petal length with points colored by species (setosa, versicolor, virginica); setosa forms a tight cluster at low petal length while versicolor and virginica trend upward, shown with a customized minimal theme."
...
The visualization in @fig-custom-theme reveals several key insights:
...
#| fig-cap:
#|   - "ColorBrewer palettes for ecological data"
#|   - "Viridis palette display"
#|   - "Scatter plot with Set1 palette"
#|   - "Scatter plot with Viridis palette"
#|   - "Continuous color scale example"
#| fig-alt:
#|   - "Stacked display of three ColorBrewer palettes (sequential YlGn, diverging BrBG, diverging RdYlBu) shown as horizontal color swatches in a 4-row layout."
#|   - "Horizontal swatch of the eight-color viridis palette rendered with scales::show_col, illustrating its perceptually uniform progression from dark purple to yellow."
#|   - "Iris sepal-length vs petal-length scatter colored by species using the qualitative Set1 palette; three species clusters separate clearly."
#|   - "Same iris scatter recolored with the discrete viridis palette, demonstrating a colorblind-friendly alternative to Set1."
#|   - "Iris scatter where point color encodes petal width on a continuous viridis scale, adding a third quantitative dimension to the plot."
...
#| fig-cap: "Multi-panel figure combining boxplots and density plots"
#| fig-alt: "Patchwork 2x2 figure of iris morphology: top row holds boxplots of sepal length and petal length by species; bottom row holds species-colored density curves of the same two variables."
...
```{r}
#| label: fig-interactive-plotly
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Interactive scatter plot of iris dataset"
#| fig-alt: "Interactive plotly scatter plot of iris sepal length vs petal length with points colored by species and hover tooltips; species form distinct groups across the plot area."
library(plotly)
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Ecological study sites across different habitat types"
#| fig-alt: "Interactive leaflet map (or static fallback) of five Bay Area ecological study sites, with circle markers colored by habitat (forest, wetland, grassland, alpine, coastal) and sized by species count."
library(leaflet)
library(ggplot2)
library(knitr)
