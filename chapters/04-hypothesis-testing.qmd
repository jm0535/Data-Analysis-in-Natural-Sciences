...
- A large p-value (> 0.05) indicates insufficient evidence to reject the null hypothesis.
...
- A **high p-value does *not* mean the null hypothesis is true** — only that the data do not provide enough evidence to reject it. Failing to reject H₀ is not the same as proving H₀.
...
The analysis in @fig-one-sample-ttest provides several key insights:
...
The confidence interval analysis (see @tbl-confidence-interval and @fig-confidence-interval) reveals:
